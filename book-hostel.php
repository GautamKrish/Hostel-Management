...
$query="insert into  booking(roomno,seater,feespm,foodstatus,stayfrom,duration,course,regno,firstName,middleName,lastName,gender,contactno,emailid,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresState,corresPincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('iiiisissssssisississsi',$roomno,$seater,$feespm,$foodstatus,$stayfrom,$duration,$course,$regno,$fname,$mname,$lname,$gender,$contactno,$emailid,$emcntno,$gurname,$gurrelation,$gurcntno,$caddress,$ccity,$cstate,$cpincode);
$stmt->execute();
				...
				<div class="row">
					<div class="col-md-12">
						...
						<div class="row">
							...
							<?php
$uid=$_SESSION['login'];
							 $stmt=$mysqli->prepare("SELECT emailid FROM booking WHERE emailid=? ");
...
<?php	
$aid=$_SESSION['id'];
	$ret="select * from hostelite where id=?";
...
<button class="btn btn-danger" type="submit" style="font-size:25px">Cancel</button>
<input type="submit" name="submit" style="font-size:25px" Value="Register" class="btn btn-info">
</div>
</form>
