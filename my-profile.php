...
$query="update  hostelite set regNo=?,firstName=?,middleName=?,lastName=?,gender=?,contactNo=? where id=?";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('sssssii',$regno,$fname,$mname,$lname,$gender,$contactno,$aid);
$stmt->execute();
$query="update booking set firstName=?,middleName=?,lastName=?,gender=?,contactno=? where regno=?";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('ssssis',$fname,$mname,$lname,$gender,$contactno,$regno);
$stmt->execute();
...
<input type="submit" name="update" style="font-size:25px" Value="Update Profile" class="btn btn-primary">
</div>
</form>
